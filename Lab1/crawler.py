...
class LIFO_Cycyle_Policy:
    ...
    def getURL(self, c, iteration):
        if len(self.queue) == 0:
            updateIncomingURLs()
            self.queue.extend(list(c.seedURLs))
            self.fetched.clear()
            return None
        else:
    ...
    def getURL(self, c, iteration):
        if len(self.queue) == 0:
            for url in self.fetched:
                c.incomingURLs[url] = len(c.incomingURLs) + 1
            self.queue.extend(list(c.seedURLs))
            self.fetched.clear()
            return None
        else:
            element = numpy.random.choice(self.queue)
            self.fetched.add(element)
            self.queue.remove(element)
            return element
...
class Container:
    def __init__(self):
        ...
        self.generatePolicy = LIFO_Cycyle_Policy(self)
...
def main():
    ...
    for iteration in range(c.iterations):
        ...
        if len(c.generatePolicy.queue) <= 0:
            c.URLs= set()
            c.generatePolicy.queue.extend(c.seedURLs)
...
def getNormalisedURLs(retrievedURLs):
    normalised = set([url.lower() for url in retrievedURLs])
    result = set()
    for x in normalised:
        if x not in result:
            result.add(x)
...
def getFilteredURLs(c, retrievedURLs):
    ...
    return toLeft.difference(c.URLs)
